...
NimLearner::NimLearner(unsigned startingTokens) : g_(true) {
	...
	for (unsigned j = startingTokens; j >= 1; j--)
	{
		for (int i = 1; i <= 2; i++)
		{
			total = s1 + to_string(i) + s3 + to_string(j);
			a = g_.getVertexByLabel(total);
			total = s1 + to_string(3 - i) + s3 + to_string(j - 1);
			b = g_.getVertexByLabel(total);
			check = g_.insertEdge(a, b);
			g_.setEdgeWeight(a, b, 0);
		}
	}
	for (unsigned j = startingTokens; j >= 2; j--)
	{
		for (int i = 1; i <= 2; i++)
		{
			total = s1 + to_string(i) + s3 + to_string(j);
