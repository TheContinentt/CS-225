...
/*	:my_stem (new Rectangle (pistil, STEM_COLOR, STEM_WIDTH, STEM_HEIGHT)),
...
void Flower::drawPistil(PNG* canvas, const Vector2& center, int x, int y) const
{
    Circle petal(center, PISTIL_COLOR, PISTIL_RADIUS);
    petal.set_center(Vector2(center.x() + x, center.y() + y));
    petal.draw(canvas);
    petal.set_center(Vector2(center.x() + x, center.y() - y));
    petal.draw(canvas);
...
void Flower::draw(PNG* canvas) const
{
    ...
    //pistil.draw(canvas);
    leaf.draw(canvas);
	for (int x = 0; x < PETAL_RADIUS; x++) {
        const double radius2 = PETAL_RADIUS * PETAL_RADIUS;
        const int y = static_cast<int>(sqrt(radius2 - (x * x)));
        drawPistil(canvas, pistil.center(), x, y);////////
    }
