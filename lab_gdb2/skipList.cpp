...
SkipList::SkipList()
{
    SkipNode* headd = new SkipNode();
    SkipNode* taill = new SkipNode();
this->head = headd;
this->tail = taill;
    this->head->nodePointers[0].next = tail;
    this->tail->nodePointers[0].prev = head;
//head->nodePointers[0].prev = NULL;
//tail->nodePointers[0].next = NULL;
    this->head->key = INT_MIN;
    this->head->value = HSLAPixel();

    this->tail->key = INT_MAX;
    this->tail->value = HSLAPixel();
//head->(INT_MIN, HSLAPixel(), 1);
//(*tail).(INT_MAX, HSLAPixel(), 1);
    int listHeight = 1;
    int length = 0;
...
void SkipList::insert(int key, HSLAPixel value)
{
    ...
    while (traverse->nodePointers[level].next != NULL && (level >= 0))
    {
        int nextKey = traverse->nodePointers[level].next->key;
	SkipNode *temp = traverse->nodePointers[level].next;
        if (key < nextKey)
        {
		/*SkipNode *i = new SkipNode(key, value, level);
		traverse->nodePointers[level].next = i;
		i->nodePointers[level].next = temp;
		temp->nodePointers[level].prev = i;
		i->nodePointers[level].prev = traverse;*/
            level--;
            if(level < 0 )
                break;
        }
