                            ...
                            queue<TreeNode*>& mergeQueue)
{
   ...
   TreeNode* smallest = NULL;
	...
	if (singleQueue.empty() == 1 && mergeQueue.empty() == 1)
		return NULL;
	else if (singleQueue.empty() == 1 && mergeQueue.empty() != 1)
	{
		TreeNode* store = mergeQueue.front();
		mergeQueue.pop();
		smallest = store;
	}
	else if (singleQueue.empty() != 1 && mergeQueue.empty() == 1)
	{
		TreeNode* store = singleQueue.front();
		singleQueue.pop();
		smallest = store;
	}
	else if (singleQueue.front() == NULL && mergeQueue.front() == NULL)
		return NULL;
	else if (singleQueue.front() == NULL && mergeQueue.front() != NULL)
	{
		TreeNode* store = mergeQueue.front();
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
	...
	{
		TreeNode *create = new TreeNode(frequencies[i]);
		singleQueue.push(create);
		i++;
	}
	while (singleQueue.size() + mergeQueue.size() != 1)
	{
		TreeNode *prev = removeSmallest(singleQueue, mergeQueue);
		TreeNode *curr = removeSmallest(singleQueue, mergeQueue);
		int frcy = prev->freq.getFrequency() + curr->freq.getFrequency();
		TreeNode *create = new TreeNode(frcy);
		create->left = prev;
		create->right = curr;
		mergeQueue.push(create);
	}
	if (singleQueue.size() == 1)
		root_ = singleQueue.back();
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
	...
	{
		char temp = bfile.getNextByte();
		Frequency store = Frequency(temp, 0);
		TreeNode *readcreate = new TreeNode(store);
		return readcreate;
	}
	else
	{
		TreeNode *readcreate = new TreeNode(0);
		readcreate->left = HuffmanTree::readTree(bfile);
		readcreate->right = HuffmanTree::readTree(bfile);
		return readcreate;
	}
	return NULL;
}
