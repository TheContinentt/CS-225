...
V BTree<K, V>::find(const BTreeNode* subroot, const K& key) const
{
	...
	if (subroot->elements[first_larger_idx] == key && flag == 0)
	{
		retval = subroot->elements[first_larger_idx].value;
		return retval;
	}
	...
	{
		retval = find(subroot->children[first_larger_idx], key);
	}
	return retval;
...
void BTree<K, V>::split_child(BTreeNode* parent, size_t child_idx)
{
    ...
    /* TODO Your code goes here! */
	//size_t parent_idx = parent->children.size() - 1;
	DataPair temp = child->elements[mid_elem_idx];
...
//	parent->children[parent_idx] = 
	...
	{
		//(new_right->children).assign(mid_elem_itr + 1, new_left->children.end());
...
void BTree<K, V>::insert(BTreeNode* subroot, const DataPair& pair)
{
	...
	if (subroot->elements[first_larger_idx] == pair.key && flag == 0)
	{
		return;
	}
	...
	{
		split_child(subroot, insertion_idx(subroot->elements, pair));
	}
	return;
}
