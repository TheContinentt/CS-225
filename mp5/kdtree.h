...
class KDTree
{
  public:
    ...
    Point<Dim> Quickselect(int left, int right, int middile, int dimension);

    int Partition(int left, int right, int middleindex, int dimension);

    void KDTreehelper(int left, int right, int middle);

    Point<Dim> findNearestNeighborhelper(const Point<Dim>& query, int left, int right, int middle, int dimension) const;
